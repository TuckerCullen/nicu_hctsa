...
def MD_hrv_classic(y):
    ...
    F, Pxx = signal.periodogram(y, window= np.hanning(N), nfft=257) #hanning confirmed to do the same thing as hann in matlab, periodogram() is what differs

    # calculate spectral measures such as subband spectral power percentage, LF/HF ratio etc.
    print("hann: ", np.hanning(N))
    print("pxx: ", Pxx)
    print("F: ", F)
