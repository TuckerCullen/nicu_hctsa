...
from operations import CO_FirstZero as fz
...
def PH_Walker(y, walkerRule='prop', walkerParams=np.array([])):
    ...
    out['w_std'] = np.std(w)
    out['w_ac1'] = ac.CO_AutoCorr(w, 1, method='timedomainstat') # this function call in MATLAB uses method='Fourier', but we don't have that case implemented yet in autoCorr, however this seems to output the same thing
